...
def raz_del(lst,rwin):
    ...
    try:
        ...
        if ww.find_couple(ww.dbout('problem'), \
            ...
            lst.delete(or_ind)
            tkbox.showinfo('Успешно','Раздел "' + old_raz[0] + '" удален из дисциплины "' \
                           + old_raz[2][0] + '"', parent = rwin)
            lst.selection_set(END)
...
def raz_prob(lst, pwin):
    try:
        ...
        prob_label = Label(prob,text = 'Задания раздела "' + raz[0] + '" дисциплины "' \
                               + ww.dbout('discipline')[raz[1]][0] + '"', font = ww.font_other)
        ...
        for item in problem:
                prob_list.insert(END, [ ww.types[problem[item][2]][1] ,\
                                 '-', problem[item][0] ] )
...
##        print(ww.find(ww.dbout('problem'), list(new_prob)))
        ...
        if new_prob[0] == '':
            ...
            tkbox.showwarning('Ошибка','Такое задание уже есть в разделе "' \
                              + praz + '"', parent = rwin)
            ...
            if tp[1] not in ans_type:
                ...
                tkbox.showinfo('Успешно','Задание добавлено в раздел "' \
                               + praz + '"', parent = rwin)
